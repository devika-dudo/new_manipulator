...
def generate_launch_description():
    ...
    package_name_gazebo = 'new_manipulator'
    package_name_moveit_config = 'new_manipulator_moveit_config'
    ...
    srdf_file_path = 'config/new_manipulator.srdf'
    ...
    print("=" * 80)
    print("DEBUG: File paths being used:")
    print(f"URDF: {urdf_model_path}")
    print(f"SRDF: {srdf_model_path}")
    print(f"File exists - URDF: {os.path.exists(urdf_model_path)}")
    print(f"File exists - SRDF: {os.path.exists(srdf_model_path)}")
    print("=" * 80)
    ...
    # Temporarily skip robot_description to see if that's the issue
    print("DEBUG: Creating MoveItConfigsBuilder...")
    
    try:
        moveit_config = (
            MoveItConfigsBuilder("new_manipulator", package_name=package_name_moveit_config)
            .robot_description(file_path=urdf_model_path)  # This line might be the problem
        ...
        print("DEBUG: MoveItConfigsBuilder succeeded!")
    except Exception as e:
        print(f"DEBUG: MoveItConfigsBuilder failed with error: {e}")
        import traceback
        traceback.print_exc()
        raise
